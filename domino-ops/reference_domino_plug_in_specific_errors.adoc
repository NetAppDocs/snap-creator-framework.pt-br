...
Por padrão, uma operação de backup falhará se um ou mais bancos de dados estiverem em um estado inconsistente ou corrompido. Você pode forçar o plug-in a prosseguir com operações de backup apesar dos erros, conforme descrito na etapa link:task_using_the_gui_to_create_a_configuration_file.html["tarefa usando a gui para criar um arquivo de configuração"].
...
O plug-in do Domino não pode funcionar com SNAP_TIMESTAMP_ONLY
 a| 
Para obter mais informações, consulte a etapa link:task_using_the_gui_to_create_a_configuration_file.html["tarefa_usando_a_interface_de_usuário_para_criar_um_arquivo_de_configuração"].
