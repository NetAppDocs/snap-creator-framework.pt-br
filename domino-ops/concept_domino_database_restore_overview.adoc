...
As restaurações de volume e as restaurações de arquivo único geralmente têm diretórios de destino diferentes. Você especifica o diretório de destino no parâmetro Diretório de Restauração de Dados no arquivo de configuração de backup e restauração (consulte a etapa link:task_using_the_gui_to_create_a_configuration_file.html["tarefa usando a gui para criar um arquivo de configuração"]):
