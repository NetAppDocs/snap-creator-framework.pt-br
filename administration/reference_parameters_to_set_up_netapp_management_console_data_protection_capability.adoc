...
NTAP_DFM_SNAPSHOT_FORMAT
 a| 
 a| 
Configuração opcional para o formato das cópias de Snapshot secundárias ao usar o recurso de proteção de dados do NetApp Management Console. Esta opção requer o OnCommand 5.0 ou posterior. Variável obrigatória:

* Carimbo de data/hora: %T


Variáveis opcionais:

* Tipo de retenção: %R
* Rótulo do conjunto de dados: %L
* Controlador de armazenamento: %H
* Nome do volume: %N
* Dados específicos da aplicação: %A


Caso essa opção não esteja definida, será utilizada a convenção de nomenclatura padrão para o recurso de proteção de dados do NetApp Management Console. A convenção de nomenclatura só pode ser definida no momento da criação do conjunto de dados.
