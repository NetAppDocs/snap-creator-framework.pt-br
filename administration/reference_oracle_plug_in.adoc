...
(S|N)
 a| 
Se definido como "Sim" durante a execução do backup offline do Oracle, o sistema utiliza o mecanismo de interrupção de desligamento para garantir que o banco de dados seja desligado em um estado consistente. Os passos para o desligamento do parâmetro ORACLE_SHUTDOWNABORT são os seguintes:
